...
test('plan → parallel research → critic rejection cycle → synthesis', async () => {
  ...
  expect(trace.some((s) => s.vetoed.some((v) => v.system === 'tokenBudget'))).toBe(true);
  expect(trace.every((s) => s.runs.every((r) => r.system !== 'tokenBudget'))).toBe(true);
...
test('a tiny token budget halts the team gracefully with partial findings', async () => {
  ...
  expect(runsByStep(world)).toEqual([
    ['planner'],
    ['researcher:investigate', 'researcher:investigate', 'tokenBudget'],
  ]);
  const trace = world.getTrace();
  const stampStep = trace.findIndex((s) =>
    s.applied.some((c) => c.component === 'research:BudgetExceeded'),
  );
