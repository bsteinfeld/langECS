...
| **Budget / watchdog** | Stop unbounded spend gracefully | a ledger with an append reducer, a `when`-guarded watchdog, and `BudgetExceeded` that every model system excludes with `Not()` — stopping is state, not an exception | [research-team](research-team/) |
| **Narration** | Make implicit control flow readable | stdlib `Phase`/`Goal` — components with *no* scheduling role, read by `narrateWorld` for humans and devtools | [research-team](research-team/) |
